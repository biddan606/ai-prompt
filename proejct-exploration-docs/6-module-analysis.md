...
**입력**: 1단계의 `{{진입점}}`, 0단계의 `{{프로젝트_유형}}`
**출력**: 
- `{{공개_API}}`: [{이름, 타입, 시그니처, 설명}] (일반 프로젝트)
- `{{커맨드_인터페이스}}`: [{커맨드명, 옵션, 인자, 설명}] (CLI 도구)
...
### 2-2. Export 목록 추출 (애플리케이션/라이브러리/프레임워크)
...
### 2-4. CLI 커맨드 인터페이스 분석 (CLI 도구인 경우)
...
**입력**: 2단계의 `{{공개_API}}` 또는 `{{커맨드_인터페이스}}`, 1단계의 `{{파일_목록}}`
**출력**: 
- `{{내부_구조}}`: [{파일, 역할, 주요_함수}]
- `{{핵심_로직}}`: 주요 구현 설명
...
### 5-2. 문서 작성
아래 `<format>` 형식에 따라 작성
- `{{프로젝트_유형}}`이 CLI 도구 → "3-B. 커맨드 인터페이스" 형식 사용
- 그 외 → "3-A. 공개 API" 형식 사용
...
<!-- 
프로젝트 유형에 따라 아래 두 형식 중 하나를 선택:
- 애플리케이션/라이브러리/프레임워크 → "3-A. 공개 API" 사용
- CLI 도구 → "3-B. 커맨드 인터페이스" 사용
선택하지 않은 섹션은 삭제합니다.
-->

#### 3-A. 공개 API (애플리케이션/라이브러리/프레임워크용)
