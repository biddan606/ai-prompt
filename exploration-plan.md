...
| 프로젝트 유형 | Phase 1 초점 | Phase 2 초점 | Phase 3 초점 |
|--------------|-------------|-------------|-------------|
| **애플리케이션** | 진입점 분석 → 핵심 비즈니스 흐름 1-2개 추적 | 주요 모듈별 역할 파악 | 세부 구현, 엣지케이스 |
| **라이브러리** | 공개 API 목록화 → 테스트에서 사용 예제 추출 | 핵심 내부 구현 원리 | 확장/커스터마이징 방법 |
| **프레임워크** | 생명주기 이해 → 기본 규약 파악 | 미들웨어/플러그인 체인 분석 | 확장 포인트, 훅 시스템 |
| **CLI 도구** | 진입점 → 메인 커맨드 실행 흐름 | 서브커맨드 구조 분석 | 옵션/플래그 처리, 설정 파일 |
...
```markdown
Phase 1: 사용법 이해
- [ ] 공개 API 목록화 (index.ts / __init__.py)
- [ ] 핵심 타입/인터페이스 분석
- [ ] 사용 예제 추출 (테스트 코드에서)
